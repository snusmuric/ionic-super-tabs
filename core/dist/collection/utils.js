...
export const scrollEl = (el, x, y, duration = 300) => {
    if (duration <= 0) {
        requestAnimationFrame(() => {
            el.scrollTo(x, y);
        });
...
export function checkGesture(newCoords, initialCoords, config) {
    if (!initialCoords) {
        return false;
    }
...
export function getScrollX(el, delta) {
    return el.scrollLeft + (typeof delta === 'number' ? delta : 0);
}
export function getScrollY(el, delta) {
    return el.scrollTop + (typeof delta === 'number' ? delta : 0);
}
export function getNormalizedScrollX(el, delta) {
    const minX = 0;
...
export function debugLog(config, tag, vals) {
