...
const SuperTabsComponent = class {
    constructor(hostRef) {
        ...
        this._config = utils.DEFAULT_CONFIG;
        this.initAttempts = 0;
    ...
    async setConfig(config) {
        ...
        this._config = Object.assign(Object.assign({}, utils.DEFAULT_CONFIG), config);
    }
    propagateConfig() {
     ...
     */
    debug(...vals) {
        utils.debugLog(this._config, 'tabs', vals);
    }
    render() {
        // Render 3 slots
        ...
        return (index.h(index.Host, null, index.h("slot", { name: "top" }), index.h("slot", null), index.h("slot", { name: "bottom" })));
    }
    get el() { return index.getElement(this); }
...
const SuperTabsContainerComponent = class {
    constructor(hostRef) {
        ...
        this.scrollWidth = 0;
        this.clientWidth = 0;
        this.activeTabIndexChange = index.createEvent(this, "activeTabIndexChange", 7);
        this.selectedTabIndexChange = index.createEvent(this, "selectedTabIndexChange", 7);
    ...
    moveContainer(scrollX, animate) {
        utils.scrollEl(this.el, scrollX, 0, animate ? this.config.transitionDuration : 0);
        return Promise.resolve();
    }
    /** @internal */
    ...
    async scrollToTop() {
        ...
        this.queue.read(() => {
            ...
            current.getRootScrollableEl()
                .then(el => {
                if (el) {
                    utils.scrollEl(el, 0, 0, this.config.transitionDuration);
                }
            });
        });
    }
    ...
    async onTouchStart(ev) {
        ...
        if (this.config.shortSwipeDuration > 0) {
            this.initialTimestamp = utils.getTs();
        }
    ...
    async onTouchMove(ev) {
        ...
        const scrollLeft = utils.getScrollX(this.el);
        const scrollX = utils.getNormalizedScrollX(this.el, deltaX);
    ...
    async onTouchEnd(ev) {
        ...
        const coords = utils.pointerCoord(ev);
        const deltaTime = utils.getTs() - this.initialTimestamp;
    ...
    calcSelectedTab() {
        ...
        const scrollX = Math.max(minX, Math.min(maxX, utils.getScrollX(this.el)));
     ...
     */
    debug(...vals) {
        utils.debugLog(this.config, 'container', vals);
    }
    render() {
        return index.h("slot", null);
    }
...
exports.super_tab = SuperTabComponent;
exports.super_tabs = SuperTabsComponent;
exports.super_tabs_container = SuperTabsContainerComponent;
