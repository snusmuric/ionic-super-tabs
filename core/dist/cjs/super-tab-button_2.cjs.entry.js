...
const utils = require('./utils-2e80317f.js');
...
const SuperTabButtonComponent = class {
    ...
    render() {
        return (index.h(index.Host, { role: "button", "aria-label": this.label ? this.label.innerText : false, "aria-disabled": this.disabled ? 'true' : false, "aria-selected": this.active ? 'true' : 'false', class: {
            ...
            } }, index.h("slot", null), index.h("ion-ripple-effect", { type: "unbounded" })));
    }
...
const SuperTabsToolbarComponent = class {
    ...
    moveContainer(scrollX, animate) {
        ...
        utils.scrollEl(this.buttonsContainerEl, scrollX, 0, animate ? this.config.transitionDuration : 0);
        return Promise.resolve();
    }
    onClick(ev) {
        if (!ev || !ev.target) {
    ...
    async onTouchStart(ev) {
        ...
        const coords = utils.pointerCoord(ev);
    ...
    async onTouchMove(ev) {
        ...
        const coords = utils.pointerCoord(ev);
        if (!this.isDragging) {
            const shouldCapture = utils.checkGesture(coords, this.initialCoords, this.config);
            if (!shouldCapture) {
        ...
        // scroll container
        const scrollLeft = utils.getScrollX(this.buttonsContainerEl);
        const scrollX = utils.getNormalizedScrollX(this.buttonsContainerEl, deltaX);
    ...
    adjustContainerScroll(animate) {
        ...
        const sp = utils.getScrollX(this.buttonsContainerEl);
     ...
     */
    debug(...vals) {
        utils.debugLog(this.config, 'toolbar', vals);
    }
    render() {
        return (index.h(index.Host, { role: "navigation", class: this.hostCls }, index.h("div", { class: "buttons-container", ref: (ref) => (this.buttonsContainerEl = ref) }, index.h("slot", null), this.showIndicator && (index.h("super-tab-indicator", { ref: (ref) => (this.indicatorEl = ref), toolbarPosition: this.el.assignedSlot.name, showPointer: this.showPointer })))));
    }
    get el() { return index.getElement(this); }
...
exports.super_tab_button = SuperTabButtonComponent;
exports.super_tabs_toolbar = SuperTabsToolbarComponent;
