...
export class SuperTabsToolbarComponent implements ComponentInterface {
  ...
  @Prop() showPointer: boolean = false
  /**
  ...
  moveContainer(scrollX: number, animate?: boolean): Promise<void> {
    if (!this.buttonsContainerEl) {
      this.debug(
        'moveContainer called before this.buttonsContainerEl was defined'
      )
      return Promise.resolve()
    }

    scrollEl(
      this.buttonsContainerEl,
      scrollX,
      0,
  ...
  async onTouchMove(ev: TouchEvent) {
    if (
      ...
      this.debug(
        'onTouchMove called before this.buttonsContainerEl was defined'
      )
      return Promise.resolve()
    }
    ...
    if (!this.isDragging) {
      const shouldCapture = checkGesture(
        coords,
        this.initialCoords!,
        this.config!
      )
  ...
  private adjustContainerScroll(animate: boolean) {
    if (!this.buttonsContainerEl) {
      this.debug(
        'adjustContainerScroll called before this.buttonsContainerEl was defined'
      )
      return
    }
  ...
  private alignIndicator(index: number, animate: boolean = false) {
    ...
    if (!this.indicatorEl) {
      this.debug(
        'alignIndicator called before this.buttonsContainerEl was defined'
      )
      return
    }
    ...
    requestAnimationFrame(() => {
      ...
      this.indicatorEl!.style.setProperty(
        '--st-indicator-position-x',
        this.indicatorPosition + 'px'
      )
      this.indicatorEl!.style.setProperty(
        '--st-indicator-scale-x',
        String(this.indicatorWidth! / 100)
      )
      this.indicatorEl!.style.setProperty(
        '--st-indicator-transition-duration',
        this.isDragging ? '0' : `${this.config!.transitionDuration}ms`
      )
    })
  }
    ...
    return (
      <Host role="navigation" class={this.hostCls}>
        <div
          ...
          ref={(ref: any) => (this.buttonsContainerEl = ref)}
        >
          <slot />
          {this.showIndicator && (
            <super-tab-indicator
              ref={(ref: any) => (this.indicatorEl = ref)}
